...
class BookTransitionHolder(
    ...
    private fun setError(error: Throwable) {
        ...
        val retryBtn = ReaderButton(context).apply {
            viewer = this@BookTransitionHolder.viewer as PagerViewer?
            wrapContent()
            setText(R.string.action_retry)
            setOnClickListener {
                val toChapter = transition.to
                if (toChapter != null) {
                    this@BookTransitionHolder.viewer.activity.requestPreloadChapter(toChapter)
                }
            }
        }
