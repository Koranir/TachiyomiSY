...
class BookPageHolder(
    ...
    private fun mergePages(imageStream: InputStream, imageStream2: InputStream?): InputStream {
        ...
        if (imageStream2 == null) {
            if (imageStream is BufferedInputStream && ImageUtil.isWideImage(imageStream) &&
                viewer.config.centerMarginType and BookConfig.CenterMarginType.WIDE_PAGE_CENTER_MARGIN > 0 &&
        ...
        val isLTR = (viewer !is R2LBookViewer) xor viewer.config.invertDoublePages

        imageStream.close()
        imageStream2.close()

        val centerMargin = if (viewer.config.centerMarginType and BookConfig.CenterMarginType.DOUBLE_PAGE_CENTER_MARGIN > 0 &&
            !viewer.config.imageCropBorders
        ) {
    ...
    private fun splitInHalf(imageStream: InputStream): InputStream {
        var side = when {
            viewer is L2RBookViewer && page is InsertPage -> ImageUtil.Side.RIGHT
            viewer !is L2RBookViewer && page is InsertPage -> ImageUtil.Side.LEFT
            viewer is L2RBookViewer && page !is InsertPage -> ImageUtil.Side.LEFT
            viewer !is L2RBookViewer && page !is InsertPage -> ImageUtil.Side.RIGHT
        ...
        val sideMargin = if ((viewer.config.centerMarginType and BookConfig.CenterMarginType.DOUBLE_PAGE_CENTER_MARGIN) > 0 &&
    ...
    private fun showErrorLayout(withOpenInWebView: Boolean): ReaderErrorBinding {
        if (errorLayout == null) {
            ...
            errorLayout?.actionRetry?.viewer = viewer as PagerViewer?
            errorLayout?.actionRetry?.setOnClickListener {
                page.chapter.pageLoader?.retryPage(page)
            }
            val imageUrl = page.imageUrl
            if (imageUrl.orEmpty().startsWith("http", true)) {
                errorLayout?.actionOpenInWebView?.viewer = viewer as PagerViewer?
