...
class GLPagerAdapter(private val viewer: GLPagerViewer) {
    /**
    ...
    private var truePageHolder = mutableListOf<GLPageImageHolder>()
    ...
    fun getPageHolder(index: Int): GLPageImageHolder? {
        try {
            return truePageHolder[index]
        } catch (e: IndexOutOfBoundsException) {
            logcat(priority = logcat.LogPriority.ERROR, message = { "Holder not initialized yet" })
            return null
        }
    }
    ...
    private fun setJoinedItems(useSecondPage: Boolean = false) {
        val oldCurrent = joinedItems.getOrNull(viewer.currentPage)
    ...
    fun splitDoublePages(current: ReaderPage) {
        ...
        viewer.scope.launchUI {
            delay(100)
            viewer.setPage(viewer.currentPage)
        }
    }
}
